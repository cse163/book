...
class SimpleTestCase(unittest.TestCase):
    def setUp(self):
        ...
        # Preserve the pyplot context before calling plt.close()
    ...
    def test_labels(self):
        correct_title = "Petal Length vs Sepal Length"
        correct_x_label = "Petal Length (cm)"
        correct_y_label = "Sepal Length (cm)"
        assert (
            self.attempt_title == correct_title
        ), f'Expected the plot to have "{correct_title}" as the title, but received "{self.attempt_title}"'
        assert (
            self.attempt_x_label == correct_x_label
        ), f'Expected the plot to have "{correct_x_label}" as the x-axis label, but received "{self.attempt_x_label}"'
        assert (
            self.attempt_y_label == correct_y_label
        ), f'Expected the plot to have "{correct_y_label}" as the y-axis label, but received "{self.attempt_y_label}"'
